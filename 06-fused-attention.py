...
This is the Phil Tillet FlashAttention implementation: https://github.com/openai/triton/blob/master/python/tutorials/06-fused-attention.py
Here for reference with additional anotation

Dao-AILab : https://github.com/Dao-AILab/flash-attention/blob/main/flash_attn/flash_attn_triton.py
---------------------------------------------------------------------------------
...
def _attn_fwd_inner(acc, l_i, m_i, q,  #
    ...
    for start_n in tl.range(lo, hi, BLOCK_N, warp_specialize=warp_specialize):
        ...
        # -- compute qk ---- Loads K blocks, computes dot products with Q
        k = desc_k.load([offsetk_y, 0]).T
        qk = tl.dot(q, k)
        if STAGE == 2: #causal masking if on diagonal (Step 2)
            mask = offs_m[:, None] >= (start_n + offs_n[None, :])
            qk = qk * qk_scale + tl.where(mask, 0, -1.0e6)
            m_ij = tl.maximum(m_i, tl.max(qk, 1))
            qk -= m_ij[:, None]
        else: #otherwise causal masking not needed (Steps 1 and 3)
            m_ij = tl.maximum(m_i, tl.max(qk, 1) * qk_scale)
            qk = qk * qk_scale - m_ij[:, None]
        p = tl.math.exp2(qk)
        # -- compute correction factor -- for online softmax
        #m_i := previous running max, m_ij := new running max, alpha := ratio of the two
        alpha = tl.math.exp2(m_i - m_ij)
        l_ij = tl.sum(p, 1) # Sum of exp for current block
        # -- update output accumulator --
        if not IS_HOPPER and warp_specialize and BLOCK_M == 128 and HEAD_DIM == 128: #hardware specific
            ...
            acc = acc * alpha[:, None] #rescales all previous contributions by alpha
        # prepare p and v for the dot
        if dtype == tl.float8e5:
            v = desc_v.load([0, offsetv_y]).T
        else:
            v = desc_v.load([offsetv_y, 0])
        p = p.to(dtype) #attention weights for current block, already rescaled
        # note that this non transposed v for FP8 is only supported on Blackwell
        acc = tl.dot(p, v, acc) #computes p @ V_j and adds it to the corrected accumulator, fused mult-add
        # update m_i and l_i
